...
class seller extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model("seller_model", "dm");
		...
		$this->validation = array(
			...
			array(
				'field'	=> 'email',
				'label'	=> 'Email',
				'rules' => 'trim|required|valid_email|is_unique[seller.email]',
			),
			array(
				'field'	=> 'password',
				'label'	=> 'Senha',
	...
	public function create()
	{
		...
		if($this->input->post() && $this->form_validation->run() === TRUE){
			...
			if($this->dm->save()){
				$this->session->set_userdata('mensagem', array('mensagem' => 'Vendedor Salvo com sucesso', 'retorno' => true));
			}else{
				$this->session->set_userdata('mensagem', array('mensagem' => 'Erro ao salvar vendedor', 'retorno' => false));
			}
	...
	public function update($id)
	{
		...
		if($this->input->post() && $this->form_validation->run() === TRUE){
			...
			if($this->dm->save($id)){
				$this->session->set_userdata('mensagem', array('mensagem' => 'Vendedor editado com sucesso', 'retorno' => true));
			}else{
				$this->session->set_userdata('mensagem', array('mensagem' => 'Erro ao editar vendedor', 'retorno' => false));
			}
	...
	public function delete($id)
	{
		if($this->dm->change_status($id, '0')){
			$this->session->set_userdata('mensagem', array('mensagem' => 'Vendedor inativado com sucesso', 'retorno' => true));
		}else{
			$this->session->set_userdata('mensagem', array('mensagem' => 'Erro ao inativar vendedor', 'retorno' => false));
		}
	...
	public function active($id)
	{
		if($this->dm->change_status($id, '1')){
			$this->session->set_userdata('mensagem', array('mensagem' => 'Vendedor ativado com sucesso', 'retorno' => true));
		}else{
			$this->session->set_userdata('mensagem', array('mensagem' => 'Erro ao ativar vendedor', 'retorno' => false));
		}
